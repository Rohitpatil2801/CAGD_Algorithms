...
void OpenGLWindow::paintGL()
{
    ...
    if (mFlag == 1)
    {
        // Specify the attribute formats only if they have changed
        if (m_posAttr != -1 && m_colAttr != -1)
        {
        ...
        glDrawArrays(GL_LINE_STRIP, 0, verticesOfOrignalLine.size() / 3);
    }
    else
    {
        // Specify the attribute formats only if they have changed
        ...
        glDrawArrays(GL_LINE_STRIP, 0, verticesOfOrignalLine.size() / 2);
    }
